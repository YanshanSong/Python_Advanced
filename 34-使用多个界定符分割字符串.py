# 你需要将一个字符串分割为多个字符按，但是分隔符(还有周围的空格)并不是固定的
# string独享的split()方法只适应于非常简单的字符串分割情形
# 它并不允许有多个分隔符或者是分隔符周围不确定的空格
# 当你需要更加灵活的切割字符串的时候，最好使用re.split()方法
line = 'asdf fjdk; afed, fjek,asdf, foo'
import re
list1 = re.split(r'[;,\s]\s*', line)
print(list1) # ['asdf', 'fjdk', 'afed', 'fjek', 'asdf', 'foo']

'''
函数 re.split() 是非常实用的，因为它允许你为分隔符指定多个正则模式。比 如，在上面的例子中，分隔符可以是逗号，分号或者是空格，并且后面紧跟着任意个 的空格。只要这个模式被找到，那么匹配的分隔符两边的实体都会被当成是结果中的 元素返回。返回结果为一个字段列表，这个跟 str.split() 返回值类型是一样的。
'''
'''
...
'''
如果你不想保留分割字符串到结果列表中去，但仍然需要使用到括号来分组正则表 达式的话，确保你的分组是非捕获分组，形如 (?:...) 。比如： 
'''
